...
| **VPP API v1** ⭐推奨 | 1.3 | `/v1/vpp/{vpp_id}/` | [docs/vpp-api-v1.md](docs/vpp-api-v1.md) | [yaml](openapi/vpp-openapi-v1.yaml) | [📘 閲覧](https://enapir.github.io/enecloud-api-spec/vpp-v1.html) |
...
　　└─ VPP API v1 を主に使用（v2.2 ↔ VPP v1.3 の組み合わせ）
...
### EMS OpenAPI v2（`/v2/ems/{ems_id}/`）

| メソッド | パス | 概要 |
|---|---|---|
| POST | `/auth/refresh` | Access Token 取得 |
...
### VPP API v1（`/v1/vpp/{vpp_id}/`）

| メソッド | パス | 概要 |
|---|---|---|
| POST | `/auth/refresh` | Access Token 取得（物理 EMS API と共通） |
...
| 項目 | 内容 |
|---|---|
| 認証 | Bearer トークン 2 種（Refresh Token → Access Token、有効期限 30 日） |
| 単位 | 電力 `kW` / 電力量 `kWh` |
| 符号規則 | 受電（充電）は正 `+`、送電（放電）は負 `-` |
| 日時形式 | ISO 8601 UTC（例 `2026-06-13T10:30:00Z`） |
| ID 形式 | 16バイトUUID = 32文字hex（ハイフン無し） |
| レートリミット | 参照系 1000 回/時、制御系 200 回/時（VPP API） |
...
仕様に関するご質問・誤記のご指摘は [Issues](../../issues) からお願いします。

---

© 2026 株式会社ナピル (NAPIR Inc.)
