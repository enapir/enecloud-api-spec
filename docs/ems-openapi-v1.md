...
**OpenAPI 定義**: [`openapi/ems-openapi-v1.yaml`](../openapi/ems-openapi-v1.yaml)
...
**例**: `/status` を呼び出す場合 → `{ホスト}/v1/ems/{id}/status`
...
**注意**: `/auth/refresh` のみ `/v1/ems/{id}/` プレフィックスを**使用しません**。また認証エンドポイントはAPI本体とは**別ホスト**で提供されます。
